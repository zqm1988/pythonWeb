...
class Model(dict):
    ...
    >>>	u = User(id=10190, name='Micheal', email='orm@db.org')
    >>> r = u.insert()
    >>> u.email
    'orm@db.org'
    >>> u.passwd
    ...
    u'orm@db.org'
    >>>	r = g.delete()
    >>> len(db.select('select * from user form where id=10190'))
    0
    >>>	import json
    >>> print User().__sql__()
    hala
	...
	def update(self):
		"""
		若该字段属性的updatable为True, 代表该字段可以被更新
		...
		SQL:update `user` set `passwd`=%s, `last_modified`=%s, `name`=%s where id=%s,
			ARGS:(u'******', 124124121.1231, u'Micheal', 10190)
		"""
	...
	def delete(self):
		self.pre_delete and self.per_delete()
		pk = self.__primary_key__.name
